...
public class ItemDao extends StoreAwareMasterDataAccessFoundation {
	...
	public void newItem(Item item) {
		Assert.notNull(item.getCode());
		Assert.notNull(item.getStoreCode());
		Item found = this.getItemByItemCode(item.getStoreCode(), item.getCode());
		if (found != null) {
			throw ItemException.itemWithCodeAlreadyExist(item.getCode());
		} else {
			this.insert(item);
		}
	}
	...
	public void update(Item item) {
		Item found = this.getItemByItemCode(item.getStoreCode(), item.getCode());
		if (found == null) {
			throw ItemException.itemWithCodeNotExist(item.getCode());
		}
		found.setPrice(item.getPrice());
		found.setChangedTime(new Date());
		found.setEnabled(item.isEnabled());
		this.update(found);
	}
