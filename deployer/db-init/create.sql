...
CREATE TABLE TENANT (CODE VARCHAR(64) NOT NULL, CHANGED_TIME DATETIME, CREATED_BY BIGINT, CREATED_TIME DATETIME NOT NULL, DESCRIPTION LONGTEXT, IS_ENABLED TINYINT(1) default 0, NAME VARCHAR(128) NOT NULL UNIQUE, ADDRESS VARCHAR(512), CITY VARCHAR(128), COUNTRY VARCHAR(128), COUNTY VARCHAR(128), PROVINCE VARCHAR(128), TWON VARCHAR(128), PRIMARY KEY (CODE))
...
CREATE TABLE STORE_IMAGE (STORE_ID BIGINT NOT NULL, FORMAT VARCHAR(255), HEIGTH INTEGER, IMG_ID VARCHAR(255), WIDTH INTEGER, PRIMARY KEY (STORE_ID))
...
ALTER TABLE STORE_IMAGE ADD CONSTRAINT FK_STORE_IMAGE_STORE_ID FOREIGN KEY (STORE_ID) REFERENCES STORE (ID)
